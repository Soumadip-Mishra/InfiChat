...
const Chatbox = () => {
	...
	const { infoView, setInfoView } = useGroupStore();

	const { authUser } = useAuthStore();

	const handleTopBar = (e) => {
		setInfoView(true);
	};
	...
	return (
		<div
			className={`bg-base-100 rounded-2xl  flex flex-col  justify-between h-[100%] w-[100%] gap-2 relative ${
				infoView ? "opacity-20" : ""
			}`}
		>
			<div
				className={`flex justify-between bg-base-300 items-center rounded-2xl px-2  `}
			...
			>
				<div className="  p-2  position relative flex gap-5 items-center ">
					...
					<span className="flex justify-around overflow-hidden ">
						{selectedGroup.name}
						{groupsTyping.has(selectedGroup._id) && (
				...
				<div className="flex gap-0 w-[60%] justify-end items-center">
					...
					/>
					<div
						className=" mx-2 flex w-full items-center shrink focus-within:outline-2 outline-1  rounded-2xl p-1 bg-base-300 "
