...
import { ChevronDown, ChevronUp, InfinityIcon, Search, X } from "lucide-react";
import { useAiMessageStore } from "../../../store/aiMessageStore";
...
const AiChatbox = () => {
	...
	const { messages, aiThinking, fetchMessages, sendMessage } =
		useAiMessageStore();
	...
	const handleSend = (e) => {
		if (aiThinking) {
			toast.error("Wait till previous operation is completed");
			return;
		}
	...
	return (
		<div className="bg-base-100 rounded-2xl flex flex-col  justify-between h-[100%] gap-2 relative">
			...
			<div className="flex flex-col overflow-auto h-[100%] gap-2 bg-base-200 rounded-2xl p-2 ">
						...
						>
							{checkNewDate(item.createdAt) && (
								<div className="flex  justify-center my-3    ">
									<span className="text-shadow-[2px_2px_4px] text-shadow-primary-content text-primary-content p-2 rounded-2xl">
										{currdate}
									</span>
								</div>
							)}
				...
				{aiThinking && (
					<div className="bg-base-300 p-3 flex-col gap-2 items-center justify-center rounded-2xl wrap-break-word  mr-auto max-w-[65%]">
						<div>
							<span className="loading loading-infinity loading-xl"></span>
							<span className="text-xl"> AI is thinking </span>
						</div>
						<div className="flex justify-center">
